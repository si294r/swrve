...
foreach ($list_filename as $filename) {
    ...
    $pcmd = "psql --host=$rhost --port=$rport --username=$ruser --no-password --echo-all $rdatabase  -c \"COPY {$table_name}{$table_suffix} FROM 's3://user-db/{$folder_s3}/{$filename}.gz' CREDENTIALS 'aws_access_key_id={$aws_access_key_id};aws_secret_access_key={$aws_secret_access_key}' DELIMITER ',' IGNOREHEADER 1 ESCAPE GZIP ACCEPTINVCHARS;\"";
    $output = array();
    exec($pcmd, $output);
    echo implode("\n", $output) . "\n\n";
