...
//$db_user = 'root';
//$db_pass = 'password';
$db_name = 'ipq_reborn_userdbs_ios';
...
//    psql:
    ...
    $pcmd = "psql --host=$rhost --port=$rport --username=$ruser --no-password --echo-all $rdatabase  -c \"COPY {$table_name}_ipq_reborn_ios FROM 's3://user-db/ipq_reborn_ios/{$filename}.gz' CREDENTIALS 'aws_access_key_id={$aws_access_key_id};aws_secret_access_key={$aws_secret_access_key}' DELIMITER ',' IGNOREHEADER 1 ESCAPE GZIP;\"";
    $output = array();
    exec($pcmd, $output);
    echo implode("\n", $output) . "\n\n";
...
$ses_cmd = "aws ses send-email --from heru@alegrium.com --to heru@alegrium.com --subject \"Swrve IPQ Reborn iOS Result Import\"  --text \"$text\"";
exec($ses_cmd);
