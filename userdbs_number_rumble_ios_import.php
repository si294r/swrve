...
//$db_user = 'root';
//$db_pass = 'password';
$db_name = 'number_rumble_userdbs_ios';
...
//    psql:
    ...
    $pcmd = "psql --host=$rhost --port=$rport --username=$ruser --no-password --echo-all $rdatabase  -c \"COPY {$table_name}_number_rumble_ios FROM 's3://user-db/number_rumble_ios/{$filename}.gz' CREDENTIALS 'aws_access_key_id={$aws_access_key_id};aws_secret_access_key={$aws_secret_access_key}' DELIMITER ',' IGNOREHEADER 1 ESCAPE GZIP;\"";
    $output = array();
    exec($pcmd, $output);
    echo implode("\n", $output) . "\n\n";
