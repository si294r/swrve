...
foreach ($arr_content as $k => $value) {
    if (strpos($value, "CREATE TABLE") !== FALSE) {
        $temp = str_replace(" (", "_jellypop_ios (", $value);
...
//    psql:
    ...
    $pcmd = "psql --host=$rhost --port=$rport --username=$ruser --no-password --echo-all $rdatabase  -c \"COPY {$table_name}_jellypop_ios FROM 's3://user-db/jellypop_ios/{$filename}.gz' CREDENTIALS 'aws_access_key_id={$aws_access_key_id};aws_secret_access_key={$aws_secret_access_key}' DELIMITER ',' IGNOREHEADER 1 ESCAPE GZIP;\"";
    $output = array();
    exec($pcmd, $output);
    echo implode("\n", $output) . "\n\n";
